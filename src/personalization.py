...
from db_model import Event, EventDuration, DBSession, Tag, EventTag, EventRecommendation
...
def personalization(event_id):
    ...
    try:
        # query event info
        query_event = session.query(Event).filter(Event.id == event_id).scalar()
        ...
        sentence_embeddings = sbert_model.encode(
            event_description, show_progress_bar=True
        )

        # store event vector into db
        event_vector = (
            session.query(EventRecommendation).filter_by(event_id=event_id).first()
        )
        if not event_vector:
    ...
    try:
        ...
        vectors = []
        events_vectors = (
            session.query(EventRecommendation)
            .join(EventTag, EventTag.event_id == EventRecommendation.event_id)
        ...
        for item in cosine_result:
            ...
            result = np.array(
                [np.array(events_ids)[index_of_score], item[index_of_score]]
            ).T
        ...
        for score in k_highest_score:
            ...
            event_vector = (
                session.query(EventRecommendation)
                .filter_by(event_id=int(score[0][0]))
                .first()
            )
